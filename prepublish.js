...
// Global variables.
const ORG = 'extra-set';
const ORIGINAL = 'Original module: [extra-set].';
...
function pkgMinify(o) {
  ...
  readme = readme.replace(/(\.<br>)/, ', exported as `Set`$1');
  readme = readme.replace(/(\.<br>)[\s\S]*?(\[!\[nodef\])/, `$1${EOL}${ORIGINAL}${EOL}${CDN}${EOL}<br>${EOL}${EOL}${EOL}$2`);
  readme = readme.replace(/extra-set\.min/g, 'extra-set');
  readme += `[unpkg]: https://unpkg.com/extra-set.min${EOL}`;
  readme += `[jsDelivr]: https://cdn.jsdelivr.net/npm/extra-set.min${EOL}`;
  pkg.name += '.min';
